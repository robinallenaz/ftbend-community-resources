...
const getLikedPosts = () => {
  try {
    ...
    const localStorageLikes = JSON.parse(localStorage.getItem('likedBlogPosts') || '[]');
...
const saveLikedPosts = (likedPosts: string[]) => {
  try {
    // Save to localStorage
    localStorage.setItem('likedBlogPosts', JSON.stringify(likedPosts));
    
    // Save to session cookie (expires when browser closes)
    const cookieValue = encodeURIComponent(JSON.stringify(likedPosts));
    document.cookie = `likedBlogPosts=${cookieValue}; path=/; max-age=86400`; // 24 hours
  } catch {
    // Silent fail
  }
};
