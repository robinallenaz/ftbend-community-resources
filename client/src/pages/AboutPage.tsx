...
import { preloadLCPImage, generateSrcSet, getSizesString, getOptimizedCloudinaryUrl } from '../utils/imageOptimization';
...
export default function AboutPage() {
  ...
  }, [selectedImage, nextImage, previousImage, toggleAutoplay]);
  ...
  useEffect(() => {
    ...
    fetch('/api/public/gallery')
      ...
      .then((data: { items: GalleryItem[] }) => {
        ...
        // Set gallery items immediately - handle 404s gracefully in onError handlers
        setGallery(data.items);
        
        // Preload first image for LCP optimization
        if (data.items.length > 0) {
          const firstImage = data.items[0];
          const optimizedSrc = firstImage.filename.includes('cloudinary') 
            ? getOptimizedCloudinaryUrl(firstImage.filename, 864, 486)
            : firstImage.filename;
          
          // Preload with high priority and store cleanup function
          cleanup = preloadLCPImage(optimizedSrc, getSizesString());
        }
      })
      .catch(console.error);
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8">
      {/* Header */}
      <header className="text-center space-y-4 rounded-3xl border border-vanillaCustard/10 bg-gradient-to-br from-pitchBlack/60 via-pitchBlack/40 to-pitchBlack/30 backdrop-blur-sm p-8 shadow-soft">
        <h1 className="text-4xl font-extrabold text-vanillaCustard">About Fort Bend LGBTQIA+ Community Resources</h1>
        <p className="text-xl text-vanillaCustard/80 max-w-3xl mx-auto">
          A community-first resource hub for LGBTQIA+ folks in and around Fort Bend County, Texas.
        </p>
      </header>
      ...
      <div className="space-y-8">
          ...
          <section className="rounded-2xl border border-vanillaCustard/15 bg-pitchBlack p-6 shadow-soft">
            <div className="relative mb-4">
              <div className="text-center">
                <h2 className="text-xl font-extrabold text-vanillaCustard">Community Moments</h2>
              </div>
              {gallery.length > 1 && (
                <button
                  onClick={toggleAutoplay}
                  className="absolute right-0 top-1/2 -translate-y-1/2 flex items-center gap-2 px-3 py-1.5 rounded-lg border border-vanillaCustard/20 text-sm text-vanillaCustard/80 hover:border-vanillaCustard/40 hover:text-vanillaCustard transition-all"
              ...
              <div className="aspect-video overflow-hidden rounded-lg bg-graphite shadow-inner max-w-4xl mx-auto relative">
                <img
                  src={gallery[currentImageIndex].filename.includes('cloudinary') 
                    ? getOptimizedCloudinaryUrl(gallery[currentImageIndex].filename, 864, 486)
                    : gallery[currentImageIndex].filename}
                  srcSet={gallery[currentImageIndex].filename.includes('cloudinary') 
                    ? generateSrcSet(gallery[currentImageIndex].filename, [
        ...
        {/* Mission & Status */}
        <section className="rounded-2xl border border-vanillaCustard/15 bg-pitchBlack p-6 shadow-soft">
          <h2 className="text-2xl font-extrabold text-vanillaCustard mb-4">Our Mission</h2>
          <p className="text-vanillaCustard/80 leading-relaxed mb-4">
          ...
          <div className="rounded-2xl bg-graphite/70 p-4">
            <div className="flex items-center gap-3">
              ...
              <h3 className="text-lg font-extrabold text-vanillaCustard">Our Current Status</h3>
            </div>
            <p className="mt-2 text-vanillaCustard/80">
              We are currently a community-driven initiative working toward formal nonprofit status. This resource directory is maintained by volunteers and community members who are passionate about connecting LGBTQIA+ folks with the support they need. While we're not yet a registered 501(c)(3), we're actively working toward establishing formal nonprofit structure to better serve our community.
            </p>
          </div>
        </section>
        ...
        {/* Get Involved */}
        <section className="rounded-2xl border border-vanillaCustard/15 bg-pitchBlack p-6 shadow-soft">
          <h2 className="text-xl font-extrabold text-vanillaCustard mb-4">Get Involved</h2>
          <div className="grid md:grid-cols-3 gap-4">
            <div className="bg-graphite/30 rounded-lg p-4 border border-vanillaCustard/10">
              <h3 className="font-semibold text-vanillaCustard mb-2">
                Community Safety
              </h3>
              <p className="text-vanillaCustard/80 text-sm leading-relaxed">
                Your safety is our priority. If you encounter outdated or harmful information,{' '}
                <a 
                  href="mailto:contact@ftbend-lgbtqia-community.org?subject=Community Safety Feedback"
                  className="text-paleAmber hover:text-vanillaCustard underline transition-colors font-medium"
                >
            ...
            <div className="bg-graphite/30 rounded-lg p-4 border border-vanillaCustard/10">
              ...
              <p className="text-vanillaCustard/80 text-sm leading-relaxed">
                Know about valuable resources or services?{' '}
                <Link 
                  to="/submit" 
                  className="text-paleAmber hover:text-vanillaCustard underline transition-colors font-medium"
                >
                ...
                {' '}to help strengthen our community support network.
              </p>
            </div>
            ...
            <div className="bg-graphite/30 rounded-lg p-4 border border-vanillaCustard/10">
              ...
              <p className="text-vanillaCustard/80 text-sm leading-relaxed">
                ...
                <a 
                  href="mailto:contact@ftbend-lgbtqia-community.org?subject=Website Feedback"
                  className="text-paleAmber hover:text-vanillaCustard underline transition-colors font-medium"
                >
                  Let us know
                </a>
        ...
        <div className="rounded-2xl bg-graphite/70 p-4">
          ...
          <p className="mt-2 text-vanillaCustard/80 text-sm">
        ...
        {/* Developer Info */}
        <div className="rounded-2xl bg-graphite/70 p-4">
          <h3 className="text-lg font-extrabold text-vanillaCustard mb-3">Website Developer</h3>
          <div className="space-y-2">
            <div>
              <a
                className="inline-flex items-center gap-2 underline underline-offset-4 hover:text-paleAmber text-sm"
          ...
          <div className="relative max-w-7xl max-h-full">
            <img
              src={selectedImage.filename.includes('cloudinary') 
                ? getOptimizedCloudinaryUrl(selectedImage.filename, 1920, 1080)
                : selectedImage.filename}
              srcSet={selectedImage.filename.includes('cloudinary') 
                ? generateSrcSet(selectedImage.filename, [
            ...
            {gallery.length > 1 && (
              <>
                <button
                  type="button"
                  onClick={() => {
                    const currentIndex = gallery.findIndex((img: GalleryItem) => img._id === selectedImage._id);
            ...
            {gallery.length > 1 && (
              <div className="absolute top-4 right-4 rounded-full bg-pitchBlack/90 backdrop-blur-sm px-3 py-1 text-sm text-vanillaCustard/80">
                {gallery.findIndex((img: any) => img._id === selectedImage._id) + 1} / {gallery.length}
              </div>
            )}
